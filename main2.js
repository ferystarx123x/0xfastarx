...
// == Implementasi TOTP RFC 6238 — tanpa library eksternal
// ===================================
...
class TelegramFullController {
    ...
    // ── Helper 2FA untuk Telegram mode ──
    _get2FA() {
        if (!this._twoFA) {
            const dataDir = path.join(__dirname, 'data');
            this._twoFA = new TwoFactorAuth(dataDir);
    ...
    async handle2FAAction(chatId, action) {
        ...
        if (op === 'delete') {
            ...
            // 2FA aktif normal: minta verifikasi OTP sebelum hapus
            const secret = tfa.getSecret(level, salt);
            this.userStates.set(chatId, {
                action: 'awaiting_2fa_delete_verify',
                level,
                secret,
                attempts: 0
            });
            this.bot.sendMessage(chatId,
                `🗑️ *HAPUS 2FA ${lbl}*\n\n` +
                `Masukkan kode OTP untuk konfirmasi penghapusan:`,
                { parse_mode: 'Markdown' }
            );
    ...
    async sendBotStatus(chatId) {
        ...
        const tfa2fa = this._get2FA();
        const salt2fa = this._get2FAMasterSalt();
